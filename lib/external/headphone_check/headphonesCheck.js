...
class HeadphonesCheck {
  ...
  constructor(_parent, settings = {}) {
    ...
    this._settings = {
      callback: undefined,
      volumeSound: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_calibration.flac',
      ...
      checkType: 'huggins',
      checkVolume: 1,
      checkExample: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_example_2.flac',
      checkSounds: [
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set1_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set1_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set1_3.flac'},
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set2_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set2_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set2_3.flac'},
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set3_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set3_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set3_3.flac'},
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set4_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set4_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set4_3.flac'},
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set5_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set5_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set5_3.flac'},
        {answer: 1, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set6_1.flac'},
        {answer: 2, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set6_2.flac'},
        {answer: 3, file: 'configs/resources/audio/stimuli_HugginsPitch/HugginsPitch_set6_3.flac'},
      ],
      trialCount: 6,
      passMark: 6,
      maxAttempts: 2,
  ...
  checkHeadphones(callback, checkVolume = true, repeat) {
    ...
    if (this._checkVolume) {
      ...
      if (repeat) {
        $h('button:contains("finished")').button('option', 'disabled', false);
      }
      promise.then(() => {
        this._instruction();
      });
  ...
  _instruction() {
    ...
    if (this._settings.checkType === 'huggins' || this._settings.checkType === 'beat') {
      $h('span[data-headphones-check-target]').html(
  ...
  _trialHandler(remaining, current = 0) {
    $h('[data-headphones-check-target]').empty();
    if (!current) {
      current = 1;
      this.attemptCount++;
      this._currentRecord = {};
    ...
    $h('#headphones-dialog').dialog('option', 'title', 'Headphones check (' + current + ' of ' + this._settings.trialCount + ')');
    $h('[data-headphones-progress]')
  ...
  _createTrial(soundFile) {
    ...
    $h('[data-headphones-check-target]').html(html);
    const audioSelector = $h('audio[data-headphones-audio-group]');
    let audioCount = audioSelector.length;
    return new Promise((resolve) => {
      audioSelector.on('canplaythrough', () => {
        if (--audioCount <= 0) {
          $h('button[data-headphones-audio-control]')
              ...
              .on('click', function() {
                $h(this)
                    .prop('disabled', true)
                    .html('Playing...');
                const audioElements = $h('audio[data-headphones-audio-group=' + this.dataset.headphonesAudioGroup + ']');
                audioElements.on('ended', () => {
                  $h('button[data-headphones-audio-control]').html('Already played');
                  $h('span[data-headphones-check-stage]').addClass('disabled');
                  $h('span[data-headphones-check-stage="2"]').removeClass('disabled');
                  $h('button[data-headphones-check-response]')
                      .prop('disabled', false)
                      .on('click', function() {
                        $h('span[data-headphones-check-stage="3"]').removeClass('disabled');
                        $h('button[data-headphones-check-response]').attr('data-helper-button', '');
                        $h(this).attr('data-helper-button', 'selected');
                        $h('button:contains("continue")')
                            .prop('disabled', false)
                            .on('click', () => {
                              resolve(
                                  $h('button[data-headphones-check-response][data-helper-button="selected"]')
  ...
  _createPlayer(selector) {
    if (selector) {
      $h(selector).button('option', 'disabled', true);
    }
    const audioSelector = $h('audio[data-headphones-audio-group]');
    let audioCount = audioSelector.length;
    audioSelector
        .on('canplaythrough', function() {
          $h(this).off('canplaythrough');
          if (--audioCount <= 0) {
            $h('button[data-headphones-audio-control]')
                .html('<img alt="Play" src="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 31.7 31.9\'%3E%3Cpath d=\'M8.6,0.5C6.8-0.6,5.2,0.2,5.2,2.4v27c0,2.2,1.5,3.1,3.4,1.9L30.3,18c1.9-1.2,1.9-3,0-4.2L8.6,0.5z\' fill=\'%2303A9F4\'/%3E%3C/svg%3E">')
                .prop('disabled', false)
                .on('click', function() {
                  ...
                  console.log(this.dataset.headphonesAudioGroup);
                  for (const audio of audioElements) {
                    audio.volume = audio.dataset.headphonesVolume;
                    if (audio.paused || audio.ended) {
                      audio.play();
                      $h(this).css('border-color', '#f0f')
                      ...
                      if (selector) {
                        $h(selector).button('option', 'disabled', false);
                      }
                    } else {
                      audio.pause();
                      $h(this).css('border-color', '#03a9f4')
    ...
    return new Promise((resolve, reject) => {
      function resizeToFit() {
        dialog
            ...
            .dialog('option', 'position', {my: 'top', at: 'top', of: "#page_content"});
      ...
      $h('body').append('<div id="headphones-dialog" title="' + options.title + '">' + options.content + '</div>');
      const dialog = $h('#headphones-dialog');
      const dialogOptions = {
        width: 'auto',
        height: 'auto',
        ...
        open: resizeToFit,
        close: function() {
          $h(window).off('resize', resizeToFit);
      ...
      resizeToFit();
      $h(window).on('resize', resizeToFit);
    });
  }
}
